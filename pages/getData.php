<?php
		...
		case "searchAlergo": 			
			...
			if(isset($data["alergo"]) && !isset($data["id_user"]) && !isset($data["age1"]) && !isset($data["gender"]) && !isset($data["work"]) && !isset($data["clientFeatures"])) {
				if($_POST["regionType"] == 0) {
					...
					$str = getFeatures($data["alergo"], " > 0.1 ");
					$region = "region = ".$_POST["regionType"]." and ".$str;
				}	

				$query = "SELECT * FROM user_results r LEFT JOIN user_info i ON r.id_user = i.id_user WHERE ".$region;
				if($result = $mysqli->query($query)) {
					$row = $result->fetch_assoc();
					$mainArray = array();
					do{	
						array_push($mainArray, dataToArray($row));	
					}while ($row = $result->fetch_assoc());	
					echo json_encode($mainArray);					
				} else {
					printError($mysqli->error); 
				...
				if($result = $mysqli->query($query)) {
					...
					if(isset($data["alergo"])) {						
						getAlergoByUsers($mainArray, $data['alergo']);	
					} else {
						getAlergoByUsers($mainArray, false);	
					}
				} else {
					printError($mysqli->error); 
				}
	...
	function getAlergoByUsers($arr, $data) {
		Global $mysqli;
		$alergoResults = array();
		if(count($arr) != 0 || !is_null($arr)) {
			foreach ($arr as $key => $value) {
				$str = "";
				if($data) {
					$mas = getFeatures($data, " > 0.1 ");
					$str = " and ".$mas;
				}

				$query = "SELECT * FROM user_results WHERE id_user = ".$value['id_user'].$str;
				// echo $query;
				if($result = $mysqli->query($query)) {
					$row = $result->fetch_assoc();
					do{
						if(!is_null($row['id_user']))
							array_push($alergoResults, dataToArray($row));		
					}while ($row = $result->fetch_assoc());		
				} 
			}
