<?php
	...
	function printError($errorMsg) {
		echo json_encode(array("erro" => $errorMsg));
	}

	function checkData() {
		$status = false;
		if((isset($_POST['userInfo'])) && (isset($_FILE['file'])) && (!empty($_POST['userInfo'])) && (!empty($_FILE['file']))) {
			return !$status;
		} else {
			return $status;
		}
